...
| `memory.py` | Deprecated import path for `02_api_backend/main.py`: `global_memory` is a reset-only handle that stores nothing; `clear()` empties every per-conversation memory. |
| `query_transform.py` | Deprecated import path for node 08 evaluation: `QueryTransformer.translate_to_english()` / `reformulate_query()` delegate to `ContextManager`. |

## Configuration

Read from `02_api_backend/config.py` when it is importable (`GROQ_API_KEY`, `LLM_MODEL`, `RETRIEVER_TOP_K`, `FINAL_TOP_K`, `USE_RERANK`, `USE_MEMORY`, optional `ROUTER_MODEL`); otherwise `settings.py` defaults apply.
...
- **Unit tests** put only `03_travel_ai_agent` on `sys.path`. The provider, node 04 adapters, node 06 retriever, node 07 generator and node 08 sink are fakes injected through constructors, so the suite needs no API key, index, ML model or other node.
- **Contract tests** (`tests/test_contracts.py`) load the real node 02, 04, 06, 07 and 08 code. When a node is missing or fails to import, its test is skipped with the reason and the owning node, e.g. `node 04 external_data cannot be imported: NameError ... (owner: node 04)`.
