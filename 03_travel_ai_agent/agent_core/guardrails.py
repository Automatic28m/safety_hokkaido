...
Validator = Callable[[Any], Tuple[bool, str, Optional[str]]]


def _local_arg_check(value: Any, max_len: int, default: Optional[str] = None) -> Tuple[bool, str, Optional[str]]:
    """Strict format check used when node 04's validators cannot be imported."""
    if (value is None or (isinstance(value, str) and not value.strip())) and default is not None:
...
LOCAL_VALIDATORS: Dict[str, Validator] = {
    "weather": lambda value: _local_arg_check(value, 60),
    "disaster": lambda value: _local_arg_check(value, 60, default="Hokkaido"),
    "train": lambda value: _local_arg_check(value, 80, default="All"),
}
ARG_KEYS: Dict[str, str] = {"weather": "city", "disaster": "region", "train": "line_name"}


def _load_validators() -> Tuple[Dict[str, Validator], str]:
    """Node 04 owns the argument rules; fall back to the local format check if it cannot be imported."""
    try:
        from external_data.validation import validate_city, validate_line_name, validate_region
    except Exception:  # node 04 absent or failing to import
        return dict(LOCAL_VALIDATORS), "local"
    return {"weather": validate_city, "disaster": validate_region, "train": validate_line_name}, "node04"


ARG_VALIDATORS, VALIDATOR_SOURCE = _load_validators()
...
def validate_tool_args(
    ...
    key = ARG_KEYS[call.name]
    validator = (validators or ARG_VALIDATORS)[call.name]
    ok, cleaned, err = validator(call.args.get(key))
